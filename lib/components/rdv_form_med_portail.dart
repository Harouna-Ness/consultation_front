...
class RdvFormMedPortail extends StatefulWidget {
  final void Function() changeView;
  const RdvFormMedPortail({super.key, required this.changeView});

  @override
  State<RdvFormMedPortail> createState() => _RdvFormMedPortailState();
}
...
// Obtenir les heures de début et de fin en fonction d'un jour spécifique
  ...
  void _selectDate(BuildContext context) async {
    ...
    // Trouver le prochain jour d'intervention
    DateTime? initialInterventionDate;
    for (int i = 0; i < 7; i++) {
      DateTime dateToCheck = today.add(Duration(days: i));
      String dayOfWeek = DateFormat('EEEE').format(dateToCheck).toUpperCase();
      bool isInterventionDay = selectedMedecin!.joursIntervention
          .map((e) => e['jour'] as String)
          .contains(dayOfWeek);

      if (isInterventionDay) {
        initialInterventionDate = dateToCheck;
        break;
      }
    }
    ...
    DateTime? selectedDate = await showDatePicker(
      ...
      initialDate: initialInterventionDate ??
          today, // Utiliser le premier jour d'intervention trouvé
      firstDate: today,
